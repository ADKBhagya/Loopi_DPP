...
export default function Topbar({ title, onMenuClick, networkLabel, walletCredits }: Props) {
  ...
  const [showSecurityExplorer, setShowSecurityExplorer] = useState(false);
  ...
  return (
    <>
      <header className="fixed top-0 right-0 left-0 lg:left-[210px] h-[72px] bg-white border-b border-gray-100 flex items-center justify-between px-3 sm:px-4 lg:px-6 z-40">
        <div className="flex items-center gap-5">
          ...
          <div className="relative hidden md:block">
            ...
            <input
              ...
              className="w-[180px] sm:w-[260px] xl:w-[380px] h-10 pl-10 pr-4 rounded-xl border bg-gray-50 text-sm outline-none focus:border-[#1B5E20]"
            />
          </div>

          <h2 className="md:hidden text-sm font-bold text-gray-800">
            {title}
          </h2>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 lg:gap-3 border rounded-xl px-2 lg:px-3 h-11 hover:bg-gray-50"
            >
              ...
              <KeyboardArrowDownOutlinedIcon className="hidden sm:block" />
            </button>
      ...
      {showToast && (
        <div className="fixed top-20 right-6 bg-white border border-gray-100 shadow-xl rounded-xl px-4 py-3 flex items-center gap-3 z-[100]">
...
function ActivityItem({ color, title, desc, time }: any) {
  ...
  return (
    <div className="flex justify-between items-start">
      ...
      <p className="text-[10px] text-gray-400 whitespace-nowrap">{time}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <InputRow
        icon={<PersonOutlineOutlinedIcon />}
        label="FULL NAME"
        value="Kaushanibhagya9"
      />
      <InputRow
        icon={<MailOutlineOutlinedIcon />}
        label="EMAIL"
        value="kaushanibhagya9@gmail.com"
      />
      <InputRow
        icon={<PhoneOutlinedIcon />}
        label="PHONE"
        value="+46 70 000 0000"
      />
      <InputRow
        icon={<BusinessOutlinedIcon />}
        label="ORGANISATION"
        value="LOOPI Enterprise"
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <SessionItem
          name="Chrome · Stockholm, SE"
          status="Current"
          sub="Now — Current"
        />
        <SessionItem
          name="Safari · Helsinki, FI"
          status="Revoke"
          sub="2 hours ago"
          danger
        />
    ...
    <div className="space-y-4">
      <div className="bg-yellow-50 border border-yellow-300 rounded-xl p-4 flex gap-3">
        ...
        <p className="text-xs text-yellow-700 font-semibold leading-relaxed">
          Never share your private key. It grants full write access to the
          blockchain under your identity.
        </p>
      </div>
        ...
        <div className="w-full max-w-[520px] bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="px-5 py-4 space-y-5 max-h-[520px] overflow-y-auto">
            ...
            <CardGroup>
              ...
              <Divider />
              <Toggle
                title="Weekly Audit Exports"
                sub="PDF sent every Monday"
                disabled
              />
              <Divider />
...
function OptionCard({ active, label, icon: Icon, onClick }: any) {
  const iconColor =
    label === "Light" ? "#F59E0B" : label === "Dark" ? "#3B82F6" : "#6B7280";
...
function Density({ active, label, onClick }: any) {
  return (
    <div
      onClick={onClick}
      className={`h-10 flex items-center justify-center rounded-lg border text-sm cursor-pointer ${
        active
...
function CardGroup({ children }: any) {
  return <div className="border rounded-xl bg-gray-50 overflow-hidden">{children}</div>;
}
